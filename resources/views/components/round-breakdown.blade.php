...
<div class="card-glass overflow-hidden animate-fade-slide-up">
    {{-- Header --}}
    <div class="px-5 sm:px-6 py-3.5 border-b border-white/[0.06]">
    ...
    <div class="px-5 sm:px-6 pt-3 pb-2">
        <div class="grid grid-cols-[1fr_90px_90px] sm:grid-cols-[1fr_120px_120px] gap-3 text-center text-[10px] font-semibold uppercase tracking-[0.1em] text-white/30">
    ...
    {{-- Rows --}}
    <div class="px-5 sm:px-6 pb-4 divide-y divide-white/[0.04]">
        @foreach($breakdown as $category => $data)
            ...
            @endphp
            <div class="grid grid-cols-[1fr_90px_90px] sm:grid-cols-[1fr_120px_120px] gap-3 py-3 items-center group hover:bg-white/[0.02] transition-colors rounded-lg -mx-2 px-2">
                {{-- Category --}}
                <div class="text-sm font-medium text-white/60 group-hover:text-white/80 transition-colors">{{ $category }}</div>
                ...
                <div class="space-y-1.5">
                    @if($aTotal > 0)
                        ...
                        <div class="text-[10px] text-white/25 text-center">{{ $data['playerA']['wins'] }}-{{ $data['playerA']['losses'] }} ({{ $aTotal }})</div>
                    @else
                        <div class="text-center">
                            <span class="text-white/15 text-sm">—</span>
                        </div>
                    @endif
                </div>
                ...
                <div class="space-y-1.5">
                    @if($bTotal > 0)
                        ...
                        <div class="text-[10px] text-white/25 text-center">{{ $data['playerB']['wins'] }}-{{ $data['playerB']['losses'] }} ({{ $bTotal }})</div>
                    @else
                        <div class="text-center">
                            <span class="text-white/15 text-sm">—</span>
                        </div>
