...
<div class="space-y-2.5">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
            ...
            @endif
            <span class="text-sm font-bold {{ $isATop ? 'text-emerald-400' : 'text-white/60' }}">{{ $labelA }}</span>
        </div>
        <div class="flex items-center gap-2">
            <span class="text-sm font-bold {{ $isBTop ? 'text-emerald-400' : 'text-white/60' }}">{{ $labelB }}</span>
            @if($isBTop && !$isTie)
                <svg class="w-3.5 h-3.5 text-emerald-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M12 2v20M2 12l4-4 4 4"/>
                </svg>
    ...
    <div class="stat-bar-track">
        ...
        @if($highlight && !$isTie)
            <div class="absolute top-0 bottom-0 w-px bg-white/20" style="left: 50%"></div>
        @endif
    </div>
