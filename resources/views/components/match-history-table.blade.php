...
<div class="card-glass overflow-hidden">
    ...
    {{-- Filters --}}
    <div class="px-5 sm:px-6 py-3 border-b border-white/[0.06] bg-white/[0.01]">
        <form method="GET" action="{{ route('players.show', $player) }}" class="flex flex-wrap items-center gap-3">
            <select name="year" onchange="this.form.submit()" class="bg-white/[0.04] border border-white/[0.08] rounded-xl px-3 py-2 text-xs font-semibold text-white/60 outline-none transition-all duration-200 focus:border-sport-500/40 focus:bg-white/[0.06]">
            ...
            <select name="tournament_id" onchange="this.form.submit()" class="bg-white/[0.04] border border-white/[0.08] rounded-xl px-3 py-2 text-xs font-semibold text-white/60 outline-none transition-all duration-200 focus:border-sport-500/40 focus:bg-white/[0.06]">
            ...
            @if($selectedYear || $selectedTournamentId)
                <a href="{{ route('players.show', $player) }}" class="text-xs text-white/40 hover:text-white/70 transition-colors underline underline-offset-2">Clear</a>
            @endif
        </form>
    </div>
    ...
    @else
                ...
                <tbody class="divide-y divide-white/[0.04]">
                    @foreach($matches as $match)
                        ...
                        <tr class="hover:bg-white/[0.02] transition-colors">
                            <td class="px-5 sm:px-6 py-3 text-white/40 whitespace-nowrap">
                                {{ $match->match_date->format('Y') }}
                            </td>
                            <td class="px-5 sm:px-6 py-3 text-white/60 max-w-[200px] truncate">
        ...
        {{-- Pagination --}}
        <div class="px-5 sm:px-6 py-4 border-t border-white/[0.06]">
