    ...
    <div class="space-y-10 sm:space-y-12">
        ...
        <div id="player-filter" class="space-y-6">
            <div class="max-w-md mx-auto space-y-4">
                <div class="relative">
                    <svg class="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-300 dark:text-white/20 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
                    </svg>
                    <input type="text"
                           class="player-filter-input w-full bg-black/[0.04] dark:bg-white/[0.04] border border-black/[0.08] dark:border-white/[0.08] rounded-xl pl-11 pr-4 py-3 text-sm outline-none transition-all duration-200 placeholder:text-gray-300 dark:placeholder:text-white/20 focus:border-sport-500/40 focus:bg-black/[0.06] dark:focus:bg-white/[0.06] text-gray-700 dark:text-white/80"
                           placeholder="Buscar jugador..."
                           autocomplete="off">
                </div>
                <p class="player-filter-count text-xs text-gray-400 dark:text-white/30 text-center">{{ count($players) }} jugadores</p>
            </div>
            ...
            @else
                ...
                @foreach($players as $player)
                    ...
                    <a href="{{ route('players.show', $player) }}"
                       class="player-card card-glass p-5 sm:p-6 space-y-4 hover:border-black/20 dark:hover:border-white/20 transition-all duration-200 group"
